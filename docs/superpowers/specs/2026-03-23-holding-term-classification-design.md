...
except ValueError:  # Feb 29 -> Feb 28; long-term starts March 1
...
- Purchased 2024-02-29 -> anniversary is 2025-02-28, so long-term from 2025-03-01 onward

### Computation: `compute_lot_month()`

In `compute_lot_month()`, determine `holding_term` by comparing `month_end` against `lot.purchase_date + 1 year`. The month-end date is always used — not any disposition date — because the WHFIT expense is a trust-level calculation prorated over the month, not tied to individual sell events.
...
Note: the WHFIT expense is a pool-level calculation — when a month has mixed holding terms across lots, you cannot attribute portions of the expense to individual lots' terms. The binary classification reflects whether the entire month's expense can be reported as long-term.
...
- `_prepare`: `dataclasses.asdict()` already converts enum fields to their `.value`, so no explicit change needed. Verify with a test.
- `_reconstruct`: add enum support by detecting `Enum` subclasses via `issubclass(cls, Enum)` (with an `isinstance(cls, type)` guard) and reconstructing via `cls(data)`.

### Disposition Scope Note

`Disposition` (share sales) is intentionally out of scope. Dispositions represent actual share sales with their own gain/loss and could have different holding term logic. This feature covers only the WHFIT trust expense classification.
...
| `db/codec.py`              | Add enum deserialization support in `_reconstruct`        |
| `tests/test_compute.py`    | Test long-term, short-term, Feb 29 edge case, transition  |
| `tests/test_csv_export.py` | Test holding_term in monthly and summary CSV output       |
| `tests/test_codec.py`      | Test enum round-trip via full `ExpenseResult` in codec    |
...
5. Lot that transitions from short-term to long-term mid-year (e.g., purchased 2024-06-15, compute 2025: short-term through June, long-term from July)
6. Summary row with all long-term lots -> `long_term`
7. Summary row with mixed lots -> `short_term`
8. Codec round-trip for `YearResult` containing `ExpenseResult` with `holding_term` field
