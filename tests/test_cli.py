"""Tests for CLI commands (import-proceeds, import-trades, compute, export, lots, status)."""
...
from fbtc_taxgrinder.cli.commands import _int_stems, cli, status
...
def test_compute_missing_proceeds(project_dir):
    """Compute without proceeds should error."""
    runner = CliRunner()
    result = runner.invoke(
        cli,
        [
...
def test_export_missing_results(project_dir):
    """Export without results should error."""
    runner = CliRunner()
    result = runner.invoke(
        cli,
        [
...
def test_import_trades_missing_file(project_dir):
    """import-trades with nonexistent file should error."""
    runner = CliRunner()
    result = runner.invoke(
        cli,
        [
...
def test_status_empty(project_dir):
    """Status with no data shows 0 lots."""
    runner = CliRunner()
