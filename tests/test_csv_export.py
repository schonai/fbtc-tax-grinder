...
from fbtc_taxgrinder.models import ExpenseResult, Disposition, YearResult
...
def test_summary_aggregates_across_lots(tmp_path):
    ...
    yr = YearResult(
        year=2024,
        lot_results={
            "lot-1": [ExpenseResult(cost_basis_of_sold=Decimal("1.00"), total_expense=Decimal("2.00"), gain_loss=Decimal("3.00"), **er_kwargs)],
            "lot-2": [ExpenseResult(cost_basis_of_sold=Decimal("4.00"), total_expense=Decimal("5.00"), gain_loss=Decimal("6.00"), **er_kwargs)],
        },
        dispositions=[],
        end_states={},
